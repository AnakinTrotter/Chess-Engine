...
// Included by Anakin Trotter for engine purposes
#include <list>
/****************************************************************************
...
public:
    ...
    // Check draw rules (50 move rule etc.) no params
    bool IsDraw();
    ...
    // Check insufficient material draw rule no params
    bool IsInsufficientDraw();
...
{
public:
    // Default contructor modified by Anakin Trotter
    ChessEvaluation() : ChessRules()
    {
        z_hash = Hash64Calculate();
    }
    ...
    void GenLegalMoveListSorted( std::list<Move> &moves );

    // Stuff added by Anakin Trotter for engine purposes
    uint64_t z_hash;
    void GenLegalMoveListSortedReg( std::vector<Move> &moves );
